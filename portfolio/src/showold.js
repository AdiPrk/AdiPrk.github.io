...
// DPR / ZOOM LOCK
// ===============================
// Browser/page zoom changes window.devicePixelRatio, which can make particles appear bigger.
// We lock the internal render DPR to the DPR at page load.
const BASE_DPR = window.devicePixelRatio || 1;
const FIXED_RENDER_DPR = Math.min(BASE_DPR, 2);
...
function setupDebugHUD() {
  ...
  // Show fixed DPR so you can see zoom isn't changing internal render resolution anymore
...
function setupThree() {
  ...
  // IMPORTANT: lock DPR so browser zoom doesn't change internal render scale.
...
function buildPoints() {
  ...
  pointsMat = new THREE.ShaderMaterial({
    ...
    vertexShader: `
      ...
      // Fixed DPR (framebuffer px per CSS px)
      uniform float uFixedPixelRatio;

      // 1.0 for screen render; TRAIL_RES_SCALE for trail RT render (compensation)
      ...
      void main() {
        ...
        // Constant size in CSS px => convert to framebuffer px via fixed DPR.
        // Also compensate for trails RT downscale so points don't bloat.
        float px = max(size, uMinPointPx) * uFixedPixelRatio * uRenderScale;
        gl_PointSize = px;
      }
    `,
    fragmentShader: `
    ...
    uniforms: {
      ...
      uFixedPixelRatio: { value: FIXED_RENDER_DPR },
...
function allocateTrailRTs() {
  // IMPORTANT: RT sizes are in framebuffer pixels.
  const pr = renderer.getPixelRatio(); // fixed
...
function resize() {
  ...
  // IMPORTANT: keep DPR fixed (do NOT follow devicePixelRatio)
  renderer.setPixelRatio(FIXED_RENDER_DPR);
  ...
  // Keep HUD showing stable DPR (fixed)
  if (DBG.dprEl) DBG.dprEl.textContent = `${FIXED_RENDER_DPR.toFixed(2)} (fixed)`;
}
...
function init() {
  ...
  for (let i = 0; i < n; i++) {
    ...
    // Interpret as CSS pixels. Keep these in a tight range for crisp points.
    pSize[i] = CONFIG.MIN_POINT_PX + (Math.random() * 0.9); // ~0.95..1.85 px
...
function animate(timeStamp) {
  ...
  // Update shader uniforms (fixed DPR)
  ...
  pointsMat.uniforms.uFixedPixelRatio.value = FIXED_RENDER_DPR;
  ...
  // Keep your DPR alpha boost, but tie it to FIXED_RENDER_DPR not zoom-varying DPR.
  const pr = FIXED_RENDER_DPR;
  const dprBoost = clamp(pr * pr, 1.0, 2.0);
  ...
  for (let i = 0; i < particleCount; i++) {
    ...
    colors[i4 + 1] = gg;
    colors[i4 + 2] = bb;
    colors[i4 + 3] = clamp(baseA * depthFade * dprBoost, 0, 1);

    // Optional depth size variation. If you want PERFECT constant size always,
    // set CONFIG.Z_SIZE_VARIATION = 0 or remove this multiplier.
  ...
  if (CONFIG.TRAILS) {
    // When rendering points INTO a downscaled RT, compensate size by TRAIL_RES_SCALE
    // so the upscaled fullscreen composite doesn't make dots appear fatter.
    pointsMat.uniforms.uRenderScale.value = CONFIG.TRAIL_RES_SCALE;
    ...
    // Normal render to screen: no compensation
    pointsMat.uniforms.uRenderScale.value = 1.0;
...
// Mobile pinch zoom can trigger visualViewport resize without a normal resize.
// We still keep DPR fixed, but we DO want to re-layout camera/RT.
